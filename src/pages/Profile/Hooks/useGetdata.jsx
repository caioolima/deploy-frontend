...
const useGetdata = () => {
    ...
    try {
      ...
      if (storedBiography) {
        ...
        const res = await fetch(
          `https://server-repository.onrender.com/users/${userId}`,
          {
            headers: {
  ...
  const getGalleryImages = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://server-repository.onrender.com/${userId}/gallery`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  useEffect(() => {
    const fetchFollowingCount = async () => {
      try {
        const response = await fetch(
          `https://server-repository.onrender.com/user/${userId}/following-count`
        );
        if (response.ok) {
          const data = await response.json();
  ...
  const verifyRelationship = useCallback(async () => {
    try {
      ...
      const url = `https://server-repository.onrender.com/relationship/${user.id}/${userId}`;
  ...
  const followUser = async () => {
    try {
      const token = localStorage.getItem("token");
      const url = `https://server-repository.onrender.com/relationship`;
  ...
  const getFollowers = useCallback(async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `https://server-repository.onrender.com/user/${userId}/followers`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const getFollowing = useCallback(async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `https://server-repository.onrender.com/user/${userId}/following`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
