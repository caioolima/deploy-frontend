...
const Function = () => {
  ...
  const loadCommunityMessages = async () => {
    try {
      const response = await fetch(
        `https://server-repository.onrender.com/message/mensagens/${communityId}`,
        {
          method: "GET",
        }
      );
      ...
      const fetchUsernames = async () => {
        ...
        for (const userId of uniqueUserIds) {
          const response = await fetch(
            `https://server-repository.onrender.com/auth/user/${userId}/username`,
            {
              method: "GET",
            }
          );
  ...
  const sendMessage = async () => {
    ...
    if (messageInput.trim() !== "" && ws.readyState === WebSocket.OPEN) {
      ...
      try {
        const response = await fetch(
          `https://server-repository.onrender.com/message/enviar-mensagem/${userId}/${communityId}`,
          {
            method: "POST",
            headers: {
    ...
    try {
      ...
      for (const userId of uniqueUserIds) {
        if (!profileImages[userId]) {
          const response = await fetch(
            `https://server-repository.onrender.com/users/${userId}/profile-images`,
            {
              method: "GET",
            }
          );
  ...
  useEffect(() => {
    ...
    const ws = new WebSocket(
      "wss://websocket-deploy.onrender.com/ws/"
    );
